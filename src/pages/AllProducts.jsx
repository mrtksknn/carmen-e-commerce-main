...
const AllProducts = () => {
  ...
  return (
    <main className="relative min-h-screen bg-background text-white font-sans overflow-hidden">
      ...
      <div className="absolute top-[-5%] right-[-10%] w-[55vw] h-[55vw] bg-primary/10 blur-[130px] rounded-full pointer-events-none z-0"></div>
      <div className="absolute bottom-[20%] left-[-10%] w-[45vw] h-[45vw] bg-primary/5 blur-[120px] rounded-full pointer-events-none z-0"></div>
      ...
      <div className='max-w-[85rem] mx-auto px-6 lg:px-20 relative z-10 pt-32 pb-20'>
        ...
        <header className="text-center mb-16 flex flex-col items-center">
          ...
          <div className="flex items-center gap-3 mb-6 animate-fade-in">
            ...
            <span className="text-[0.65rem] font-extrabold tracking-[0.4em] uppercase text-primary">
        ...
        <section className="bg-[#111]/60 border border-white/5 backdrop-blur-xl rounded-2xl p-4 md:p-6 mb-16 shadow-[0_20px_40px_rgba(0,0,0,0.5)] animate-fade-in delay-200 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row gap-4 md:items-center relative z-10">
            ...
            <div className="relative flex-1 group">
              ...
              <input
                ...
                className="w-full pl-12 pr-12 py-3.5 rounded-xl bg-black/40 border border-white/5 text-white focus:outline-none focus:bg-background focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all placeholder:text-gray-600 font-light text-sm"
              />
              {searchTerm && (
                <button
            ...
            <div className="relative min-w-[220px] shrink-0 group">
              ...
              <select
                ...
                className="w-full pl-12 pr-10 py-3.5 rounded-[1.25rem] bg-black/40 border border-white/5 text-white focus:outline-none focus:bg-background focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all cursor-pointer font-light appearance-none text-sm group-hover:border-white/10"
              >
          ...
          {(searchTerm || sortBy !== 'newest') && (
            <div className="flex items-center gap-3 mt-4 pt-4 border-t border-white/5 animate-fade-in relative z-10">
              ...
              {searchTerm && (
                <button onClick={() => setSearchTerm('')} className="inline-flex items-center gap-1.5 text-xs bg-white/5 border border-white/10 text-gray-300 px-3 py-1 rounded-full hover:bg-primary/20 hover:border-primary/50 hover:text-primary transition-all">
                  "{searchTerm}" <X size={10} strokeWidth={3} />
                </button>
              )}
              {sortBy !== 'newest' && (
                <button onClick={() => setSortBy('newest')} className="inline-flex items-center gap-1.5 text-xs bg-white/5 border border-white/10 text-gray-300 px-3 py-1 rounded-full hover:bg-primary/20 hover:border-primary/50 hover:text-primary transition-all">
                  {sortBy.replace('-', ' ').toUpperCase()} <X size={10} strokeWidth={3} />
                </button>
              )}

              <button onClick={clearFilters} className="text-[10px] uppercase font-bold tracking-widest text-primary/70 hover:text-primary ml-auto transition-colors duration-300">
                Clear All
              </button>
            </div>
          )}
            ...
            <div className="col-span-full flex flex-col items-center justify-center py-24 px-4 text-center border border-white/5 bg-white/[0.02] rounded-[2rem] mt-4 animate-fade-in">
              ...
              {(searchTerm || sortBy !== 'newest') && (
                <button
                  ...
                  className="mt-6 px-6 py-2 bg-primary/20 text-primary font-bold text-[10px] tracking-widest uppercase border border-primary/30 rounded-[1.5rem] hover:bg-primary hover:text-white transition-all duration-300 shadow-[0_0_15px_rgba(168,50,41,0.3)]"
                >
